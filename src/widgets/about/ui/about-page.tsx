...
const experiences = [
  {
    ...
    achievements: [
      "학생이 캔버스에서 교구를 조작한 '풀이 과정 기록'(교사가 되감아 재생하는 데이터)의 저장 용량을 최대 405배 절감 — 3D 활동 기준 3.7MB → 9.3KB. 저장 직전 경계에서만 변환(변경분만 기록 · 압축 · 재생이 안 쓰는 데이터 제거)해 재생 엔진은 한 줄도 고치지 않고 재생 품질 손실 0 (복원 동일성 27케이스 · 전체 1,101 테스트 통과). 합성 벤치 17배가 실제 1.1배로 무너지자, 실측 진단으로 진짜 병목(재생이 읽지 않는 3D 렌더링 데이터)을 찾아 해결",
