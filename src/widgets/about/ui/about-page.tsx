...
const LENSES = [
  {
    ...
    body: `Claude Code Skills Panel — 누적 ${fmt(
      LIVE_STATS.extensionDownloads,
    )} 다운로드(Open VSX ${fmt(LIVE_STATS.openVsxDownloads)} + VS Code Marketplace ${fmt(
      LIVE_STATS.vscodeMarketplaceDownloads,
    )}) · 평점 5.0. npm 패키지(bumpist-code · claude-distill) 주간 합산 ${fmt(
  ...
  },
  {
    ...
    body:
      "저축은행중앙회·신협 등의 WCAG 2.2/3.0 접근성 감리를 컨설턴트로 수행했어요. SEO 와 접근성이 결국 같은 구조를 본다는 걸 프론트엔드 합류 전부터 알고 있었고, vercel.app 권한 0 에서 이름 검색 1위로 증명한 적 있습니다.",
  },
  {
...
const experiences = [
  {
    id: 0,
    company: "(주)코드넛",
    role: "Frontend Developer · 수학 교육 플랫폼 MathCanvas",
    period: "2026.03 - 현재",
    description:
      "정밀성이 요구되는 수학 교육 플랫폼 MathCanvas(에듀테크 콘텐츠 뷰어/리포트 도메인)에서 Vue 3 · Pinia · Three.js · FSD 기반 신규 기능 5건 · V2 리디자인 5건 출시. 성능 최적화·3D 안정화·SSOT 정착·latent bug 해소를 동시에 수행.",
    achievements: [
      "학생이 캔버스에서 교구를 조작한 '풀이 과정 기록'(교사가 되감아 재생하는 데이터)의 저장 용량을 최대 405배 절감 — 3D 활동 기준 3.7MB → 9.3KB. 저장 직전 경계에서만 변환해 재생 엔진은 한 줄도 안 고치고 품질 손실 0: ① 처음만 통째로 저장하고 이후엔 직전 상태와 비교해 달라진 값만 기록(델타) — 교구 이동 1회가 100KB → 약 40바이트로, ② 남은 데이터는 무손실 압축(gzip), ③ 재생이 실제로 읽지 않는 3D 렌더링 데이터는 제거. 합성 벤치 17배가 실제 1.1배로 무너지자 실측 진단으로 진짜 병목을 특정 (복원 동일성 27케이스 · 전체 1,101 테스트 통과)",
      "슬라이드 전환 2.8s → 0.3s (약 9×) — 이미지 버퍼를 전환 로직에서 분리한 아키텍처 리팩터",
      "3D 렌더링 '변경 시에만 렌더' 모델 전환 — 인터랙션 13fps → 51fps(약 4×), 정지 시 오버헤드 0",
      "브라우저 GPU 3D 캔버스 통합(창당 9개 → 1개 공유) — 학생 화면 백지 이슈 해소, 800+ 유닛·통합 테스트로 정적·런타임·통합 3계층 회귀 방어",
  ...
  },
  {
    ...
    description:
      "저축은행중앙회·신협 등의 WCAG 2.2/3.0 접근성 감리를 수행. 컨설턴트 시점에서 구조와 의미를 먼저 보는 사고를 익혔어요.",
    achievements: [
      "WCAG 2.2/3.0 접근성 감리(저축은행중앙회·신협) — 1차 검수 통과율 95% 달성",
      "팀 커뮤니케이션 프로토콜 표준화로 평균 재진단 사이클 2.5회 → 0~1회 감소",
      "최신 WCAG 명세 분석 사내 지식베이스 + 교차 교육으로 문의 이메일 40% 감소",
    ],
  },
  {
    id: 2,
